...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if 'contest' in params:
            ...
            user_rank_contest = { record['party']['members'][0]['handle']:user_rank[record['party']['members'][0]['handle']] for record in cres if record['party']['members'][0]['handle'] in user_rank}
            ...
            user_country = { user['handle']:user['country'] for user in res if 'country' in user }
            user_rank = { user['handle']:user['rank'] for user in res if 'rank' in user }
            user_org = { user['handle']:user['organization'] for user in res if 'organization' in user }
